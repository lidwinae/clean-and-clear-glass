...
final class SodiumModelSupport {
    ...
    private static final boolean SUPPORTED_SODIUM_LOADED = FabricLoader.getInstance()
            ...
            .map(container -> container.getMetadata().getVersion().getFriendlyString().startsWith("0.5.13"))
            .orElse(false);
    ...
    private static RenderMaterial createCompatibleNoDiffuseMaterial(RenderMaterial emitterMaterial) {
        ...
        RenderMaterial material = createNoDiffuseMaterial(renderer);

        if (isCompatible(emitterMaterial, material)) {
            return material;
        }

        // Indium can tessellate the same model alongside C&CG's direct
        // Indigo-backed Sodium capture context. Resolve its renderer only when
        // the emitter proves that an Indigo material is not compatible.
    ...
    private static RenderMaterial createNoDiffuseMaterial(Renderer renderer) {
        ...
        return renderer.materialFinder().disableDiffuse(true).find();
    }
