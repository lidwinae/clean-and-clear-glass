...
public final class VanillaConnectedGlassPaneModel
        extends BakedModelWrapper<BakedModel> {
    private static final ModelProperty<PaneConnectionKey> CONNECTIONS =
            new ModelProperty<>();
    ...
    private final ConcurrentMap<PaneConnectionKey, NeoQuadEmitter.QuadSet>
    ...
    public List<BakedQuad> getQuads(
        ...
        PaneConnectionKey key = modelData.get(CONNECTIONS);

        if (key == null) {
            key = disconnectedKey(state);
        }
    ...
    public List<BakedQuad> getQuads(
        ...
        PaneConnectionKey key = disconnectedKey(state);
    ...
    private NeoQuadEmitter.QuadSet buildQuads(
        ...
        boolean sameNorth = key.sameNorth();
        boolean sameEast = key.sameEast();
        boolean sameSouth = key.sameSouth();
        boolean sameWest = key.sameWest();

        boolean centerUp = key.centerUp();
        boolean centerDown = key.centerDown();

        boolean northUp = key.northUp();
        boolean northDown = key.northDown();

        boolean eastUp = key.eastUp();
        boolean eastDown = key.eastDown();

        boolean southUp = key.southUp();
        boolean southDown = key.southDown();

        boolean westUp = key.westUp();
        boolean westDown = key.westDown();
    ...
    private static void emitCenterOverlayFace(
        ...
        emitter.nominalFace(face)
                .cullFace(null)
                .spriteBake(material, NeoQuadEmitter.BAKE_NORMALIZED)
                .diffuseShade(false)
                .emit();
    }

    private PaneConnectionKey connectionKey(
            BlockAndTintGetter level,
            BlockPos pos,
            BlockState state
    ) {
    ...
    private PaneConnectionKey disconnectedKey(
            @Nullable BlockState state
    ) {
        boolean north = state != null
                && state.getValue(CrossCollisionBlock.NORTH);
        boolean east = state != null
                && state.getValue(CrossCollisionBlock.EAST);
        boolean south = state != null
                && state.getValue(CrossCollisionBlock.SOUTH);
        boolean west = state != null
                && state.getValue(CrossCollisionBlock.WEST);
